DRAFT MODE - IN PROGRESS - CANNO'T BE USED FOR NOW.

[_<< Previous section: GKE advanced setup in Production](/docs/gke-advanced.md)

# GKE advanced with Anthos Service Mesh setup in Production
...
CONTAINERS_REGISTRY_NAME=${CLUSTER_NAME}-containers
...
## [PA-GCP] Deploy the ASM Ingress gateway

As Platform Admin, in Google Cloud.

Deploy the ASM Ingress gateway:
```bash
kubectl apply -f samples/asm-ingress-gateway/
